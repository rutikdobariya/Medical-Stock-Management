...
if (isset($_SESSION['id']) && isset($_SESSION['email'])) {
  if (isset($_REQUEST['submit'])) {
    ...
    // $med = rtrim($_REQUEST['med'],"\0");
    // $med = json_decode(trim($med),TRUE);
    // $med = str_replace('{','[',$med);
    // $med = str_replace('}',']',$med);
    // $med = str_replace(':','=>',$med);
    // $med = explode(',',$med);
    // $ar = array('naem'=>12,'kc'=>'ds','s'=>12);
    ...
    foreach ($medarr as $key => $value) {
      ...
      if ($key == 1 and $value == 1) {
        ...
        // echo $key.'=>'.$value;
        $q1 = "SELECT * FROM medicine WHERE medicine_name = '$key'";
        $res = mysqli_query($conn, $q1);
        $res1 = mysqli_fetch_array($res);
        // print_r($res1['medicine_quantity']);
        $finalQuantity = (int)$res1['medicine_quantity'] - $value;

        $query = "UPDATE medicine SET medicine_quantity = $finalQuantity WHERE medicine_name = '$key'";
          ...
          <div class="container-fluid">
            ...
            <div class="card shadow mb-4">
              <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">ADD CUSTOMER DETAILS</h6>
              </div>
              <div class="card-body">
                <form action="customer_addSave.php" method=" post">
                  <input type="hidden" name="tamount" id="tamount" value="<?php echo $amount ?>">
                    ...
                    <table id="dataTable" width="80%" cellspacing="0" cellpadding="20" align="center">
                      <tbody>
                        <tr align="center">
                          ...
                          <td><input type="text" class="form-control form-control-user" name="customer_name" required></td>

                          <th style=" font-size: 1.2rem;">Contact No.</th>
                          <td><input type="number" class="form-control form-control-user" name="customer_contact" required></td>
                        </tr>

                        <tr align="center">
                          <th style="font-size: 1.2rem;">Email</th>
                          <td><input type="email" class="form-control form-control-user" name="customer_email" required></td>
                        </tr>
